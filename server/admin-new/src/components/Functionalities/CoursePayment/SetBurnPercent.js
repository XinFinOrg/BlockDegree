...
class SetBurnPercent extends Component {
  constructor(props) {
    super(props);
    this.state = {
      value: "",
    ...
    this.handleTokenBurnPercentChange = this.handleTokenBurnPercentChange.bind(this);
  ...
  handleTokenBurnPercentChange(event) {
    this.setState({ burnPercent: event.target.value });
  }

  handleSubmit() {
    const courseIdValid = validate("code", this.state.courseId);
    const tokenNameValid = validate("text", this.state.tokenName);
    const validPercent = validate("float",this.state.burnPercent);

    if (!courseIdValid) {
      return showNotification("danger", "Ser Burn Percent", "Invalid course id");
    }

    if (!tokenNameValid) {
      return showNotification("danger", "Set Burn Percent", "Invalid token name id");
    }

    if (!validPercent) {
      return showNotification("danger", "Set Burn Percent", "Invalid token percent");
    }

    // data is valid
    axios
      .post("/api/setCourseBurnPercent", {
        courseId: this.state.courseId.toString(),
        tokenName: this.state.tokenName.toString(),
        burnPercent: this.state.burnPercent.toString()
      })
      .then(resp => {
          ...
          <form className="form-horizontal soft-input">
            ...
            <div className="form-group">
              ...
              <div className="col-md-8">
                <input
                  type="number"
                  value={this.state.burnPercent}
                  onChange={this.handleTokenBurnPercentChange}
                  placeholder="token burn percent"
                />
              </div>
            </div>

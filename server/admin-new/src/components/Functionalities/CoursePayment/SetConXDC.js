...
class SetConXDC extends Component {
  constructor(props) {
    super(props);
  ...
  handleSubmit() {
    ...
    const xdcConfirmationValid = validate("integer", this.state.confirmation);

    if (!courseIdValid) {
      return showNotification(
        "danger",
        "Set XDC Confirmation",
        "Invalid course id"
      );
    }

    if (!xdcConfirmationValid) {
      return showNotification(
        "danger",
        "Set XDC Confirmation",
    ...
    // data is valid
    axios
      .post("/api/setXdcConfirmation", {
        courseId: this.state.courseId.toString(),
        xdcConfirmation: this.state.confirmation.toString()
      })
      .then(resp => {
        ...
        if (resp.data.status === true) {
          ...
          this.setState({
            showSuccess: true,
            successMsg: "XDC Confirmation Set",
            courseId: "",
            confirmation: ""
          });
        } else {

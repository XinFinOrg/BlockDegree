...
class ActivateAddr extends Component {
  constructor(props) {
    super(props);
    this.state = {
      walletAddr: "",
  ...
  handleSubmit() {
    ...
    if (!walletAddressValid) {
      return showNotification("danger", "Activate Wallet", "Invalid address");
    }

    if (!walletTypeValid) {
      return showNotification("danger", "Activate Wallet", "Invalid wallet type");
    }

    if (!tokenNameValid) {
      return showNotification("danger", "Activate Wallet", "Invalid token name");
    }

    if (!networkValid) {
      return showNotification("danger", "Activate Wallet", "Invalid network");
    }

    // data is valid
    axios
      .post("/api/switchWalletTo", {
